...
<li id="li-comment-{{ $item->id }}" class="comment even {{ ($item->user_id == $article->user_id) ? "bypostauthor odd" : '' }}">
    <div id="comment-{{ $item->id }}" class="comment-container">
        <div class="comment-author vcard">
            @set($hash, ($item->user) ? md5($item->user->email) : md5($item->email))
        ...
        <div class="comment-meta commentmetadata">
            <div class="intro">
                <div class="commentDate">
                    <a href="#comment-2">
                    {{ is_object($item->created_at) ? $item->created_at->format('M d, Y \a\t H:m') : '' }}</a>                        
                </div>
    ...
    @if (isset($com[$item->id]))
        <ul class="children">
            @include(config('settings.theme').'.article.comment', ['items'=>$com[$item->id]])
        </ul>
    @endif
</li>
 @endforeach
