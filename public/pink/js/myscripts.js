jQuery(document).ready(function ($) {
	...
	$('#commentform').on('click', '#submit', function (e) {
		...
		$('.wrap_result').css('color', 'green').text('Сохранение коментария').
		fadeIn(500, function () {
			...
			$.ajax({
				...
				success:function (html) {
					if (html.error) {
						...
						$('.wrap_result').css('color', 'red').append(
							'</br><strong>Ошибка: </strong>' + html.error.join('</br>')
							);
				...
				error:function () {
					$('.wrap_result').css('color', 'red').append('<br/><strong>Сохранено!</strong>>');
